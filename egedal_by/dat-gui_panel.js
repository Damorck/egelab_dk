Q3D.gui = {
  ...
  addLayersFolder: function () {
    ...
    var layersFolder = this.gui.addFolder('Kortlag');
  ...
  addCustomPlaneFolder: function () {
    ...
    var folder = this.gui.addFolder('Brugerpanel');

    // Plane color
    folder.addColor(parameters.cp, 'c').name('Farve').onChange(function (value) {
      if (customPlane === undefined) addPlane(parameters.cp.c);
      customPlane.material.color.setStyle(value);
    });

    // Plane height
    folder.add(parameters.cp, 'd').min(zMin).max(zMax).name('Niveau').onChange(function (value) {
      if (customPlane === undefined) addPlane(parameters.cp.c);
    ...
    // Plane opacity
    folder.add(parameters.cp, 'o').min(0).max(1).name('Gennemsigtighed (0-1)').onChange(function (value) {
      if (customPlane === undefined) addPlane(parameters.cp.c);
      customPlane.material.opacity = value;
    });

    // Enlarge plane option
    folder.add(parameters.cp, 'l').name('Forstør').onChange(function (value) {
      if (customPlane === undefined) addPlane(parameters.cp.c);
  ...
  addCommandsFolder: function () {
    var folder = this.gui.addFolder('Styreegenskaber');
    if (Q3D.Controls.type == "Orbit") {
      folder.add(this.parameters.cmd, 'rot').name('Auto-rotation').onChange(function (value) {
        Q3D.application.controls.autoRotate = value;
      });
    }
    folder.add(this.parameters.cmd, 'wf').name('Trådnet Mode').onChange(Q3D.application.setWireframeMode);
  },

  addHelpButton: function () {
    this.gui.add(this.parameters, 'i').name('Hjælp');
  }
};
